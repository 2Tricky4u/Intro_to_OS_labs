...
trap_init(void)
{
  ...
  SETGATE(idt[T_DIVIDE], 0, GD_KT, divide_by_zero, 0);
  SETGATE(idt[T_DEBUG], 0, GD_KT, debug, 0);
  SETGATE(idt[T_NMI], 0, GD_KT, non_maskable_int, 0);
  SETGATE(idt[T_BRKPT], 0, GD_KT, breakpoint_interrupt, 3);
  ...
  SETGATE(idt[T_STACK], 0, GD_KT, stack_segment_fault, 0);
  SETGATE(idt[T_GPFLT], 0, GD_KT, general_protection_fault, 0);
  SETGATE(idt[T_PGFLT], 0, GD_KT, page_fault, 0);
  SETGATE(idt[T_FPERR], 0, GD_KT, floating_point_exception, 0);
  SETGATE(idt[T_ALIGN], 0, GD_KT, alignement_check, 0);
  SETGATE(idt[T_MCHK], 0, GD_KT, machine_check, 0);
  SETGATE(idt[T_SIMDERR], 0, GD_KT, simd_fp_exception, 0);
...
trap_init_percpu(void)
{
  ...
  //TODO maybe use ts (global parameter beginning of file) if bug
  struct Taskstate* taskState = &(thiscpu->cpu_ts);

  // Setup a TSS so that we get the right stack
  // when we trap to the kernel.
  int offset = thiscpu->cpu_id * (KSTKSIZE + KSTKGAP);
  taskState->ts_esp0 = KSTACKTOP - offset;
  taskState->ts_ss0 = GD_KD;

  // Initialize the TSS slot of the gdt.
  gdt[(GD_TSS0 >> 3) + thiscpu->cpu_id] = SEG16(STS_T32A, (uint32_t)(taskState),
